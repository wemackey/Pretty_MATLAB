function varargout = jsetupColours(varargin) %#ok<STOUT>
%===============================================================================
% FUNCTION: 
%   setupSolarized.m
%
...
%% Monokai Pallete
monokai.black = [0,0,0] /256;
monokai.magenta = [249,38,114]/256;
monokai.cyan =[102,217,239]/256;
monokai.green =[166,226,46]/256;
monokai.orange =[253,151,31]/256;
monokai.red =[229, 34, 34]/256;
monokai.yellow =[255,232,106]/256;
monokai.blue =[63,184,205]/256;
monokai.purple = [174,129,255]/256;
monokai.darkgreen = [42,98,70]/256;
monokai.darkorange =[244,96,64]/256;
monokai.cream = [252,252,196]/256;
monokai.darkgrey = [39,40,34]/256;
monokai.midgrey =[73,72,62]/256;
monokai.lightgrey=[248,248,242]/256;
monokai.white =[242, 242, 242]/256;


%% Setup Color Scheme for Options
if strcmp(colourScheme,'light')
...
elseif strcmp(colourScheme,'dark')
    sycb = 0;           % Set 'Use system colors' checkbox to False
    txc  = sol.base0;   % Set 'Text' color
    bgc  = monokai.black;  % Set 'Background' color
    kwd  = sol.blue;    % Set 'Keywords' color
    cmt  = sol.cyan;    % Set 'Comments' color - spec calls for base1, not sure about that...
    str  = sol.magenta; % Set 'Strings' color
    ustr = sol.orange;  % Set 'Unterminated strings' color
...
%     ahi  = sol.violet;  % Set 'Automatically highlight' color
    ...
    slnb = 1;           % Set 'Show line numbers' checkbox to True
    rtlb = 1;           % Set 'Show line' checkbox in Right-hand text limit to True
elseif strcmp(colourScheme,'monokai')
    sycb = 0;           % Set 'Use system colors' checkbox to False
    txc  = monokai.lightgrey;   % Set 'Text' color
    bgc  = monokai.black;  % Set 'Background' color
    kwd  = monokai.orange;    % Set 'Keywords' color
    cmt  = monokai.cyan;    % Set 'Comments' color - spec calls for base1, not sure about that...
    str  = monokai.magenta; % Set 'Strings' color
    ustr = monokai.yellow;  % Set 'Unterminated strings' color
    scmd = monokai.yellow;  % Set 'System commands' color
    errs = monokai.red;     % Set 'Errors' color
    hyp  = monokai.blue;    % Set 'Hyperlinks' color
    warn = monokai.green;  % Set 'Warnings' color
    afhb = 0;           % Set 'Autofix highlight' checkbox to False
    afh  = monokai.blue;  % Set 'Autofix highlight' color
    ahib = 1;           % Set 'Automatically highlight' checkbox to False
    ahi  = monokai.green;  % Set 'Automatically highlight' color
    vwss = monokai.purple;   % Set 'Variables with shared scope' color
    clhb = 0;           % Set 'Highlight cells' checkbox to False
    hclb = 1;           % Set 'Highlight current line' checkbox to True
    hcl  = monokai.darkgrey;  % Set 'Highlight current line' color
    slnb = 1;           % Set 'Show line numbers' checkbox to True
    rtlb = 1;           % Set 'Show line' checkbox in Right-hand text limit to True
    % Default colour list for plotting
%     colour = [sol.blue; ...
%         monokai.orange; ...
%         monokai.green; ...
%         monokai.magenta; ...
...
%         monokai.purple; ...
%         monokai.cyan; ...
%         monokai.yellow;]; %set deafult colors for plotting
